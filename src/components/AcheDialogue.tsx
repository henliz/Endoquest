// src/components/AcheDialogue.tsx
...
interface AcheDialogueProps {
  onMenuChoice: (
    choice: 'healers_chronicle' | 'wayside_comforts' | 'guild_restoration'
  ) => void;
  onClose: () => void;
...
export function AcheDialogue({
  onMenuChoice,
  onClose,
  hasMetBefore,
  onMarkAsMet,
}: AcheDialogueProps) {
  const [step, setStep] = useState<DialogueStep>(
    hasMetBefore ? 'menu' : 'player_surprised'
  );
  const [playerApologized, setPlayerApologized] = useState(false);

  const handleContinue = () => {
    if (step === 'player_surprised') setStep('ache_responds');
    else if (step === 'ache_responds') setStep('player_question_why');
    else if (step === 'player_question_why') setStep('ache_explain_heard');
    else if (step === 'ache_explain_heard') setStep('ache_explain_screaming');
    else if (step === 'ache_explain_screaming') setStep('player_realize');
    else if (step === 'player_realize') setStep('ache_always_there');
    else if (step === 'ache_always_there') setStep('player_apologize_choice');
    else if (step === 'ache_accept_apology') setStep('ache_together');
    else if (step === 'ache_no_apology_needed') setStep('ache_together');
    else if (step === 'ache_together') setStep('player_together_accept');
    else if (step === 'player_together_accept') setStep('ache_offer_help');
  ...
  const handleChoice = (choiceId: string) => {
    if (step === 'player_apologize_choice') {
      ...
      if (choiceId === 'healers_chronicle') onMenuChoice('healers_chronicle');
      else if (choiceId === 'wayside_comforts') onMenuChoice('wayside_comforts');
      else if (choiceId === 'guild_restoration')
        onMenuChoice('guild_restoration');
      else if (choiceId === 'nothing') onClose();
    }
  };

  return (
    <>
      {/* Ache theme only while this dialogue is active */}
      <WebAudioManager track="ache" volume={0.35} />
        ...
        {step === 'menu' && (
          <VNChoiceScene
            ...
            choices={[
              ...
              { id: 'nothing', text: 'Nothing right now... but thank you.', emotion: 'Peaceful' },
            ]}
            onChoiceSelect={handleChoice}
          />
        )}
